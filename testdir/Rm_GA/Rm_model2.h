...
const int model2_M = 1000007;
int model2_result = -1;
Job model2_jobs[MAX_N];
double model2_running_time;



void model2_optimation()
{
	GRBEnv env = GRBEnv();
	...
	for (int i = 0; i < current_config.n; ++i)
	{
		...
		{
			if (model2_jobs[i].proc_time[j] > tmp)
				tmp = model2_jobs[i].proc_time[j];
		}
		max_c += tmp;
	}
	...
	for (int i = 1; i < current_config.n + 1; ++i)
	{
		...
		for (int j = 0; j < current_config.m; ++j)
		{
			if (model2_jobs[i-1].proc_time[j] > tmp)
				tmp = model2_jobs[i - 1].proc_time[j];
		}
		p[i] = model.addVar(0, tmp, 0, GRB_INTEGER);
		c[i] = model.addVar(0, max_c, 0, GRB_INTEGER);
	}
		...
		for (int i = 1; i < current_config.m + 1; ++i)
		{
			for (int j = 0; j < current_config.n + 1; ++j)
			{
				if (j != k)
					...
					tmp += xxx[i][j][k] * model2_jobs[k-1].proc_time[i-1];
			}
		}
	...
	for (int j = 1; j < current_config.n + 1; ++j)
	{
		...
		model.addConstr(p[j] <= z[j] + model2_M * (1 - u[j][0]));
		model.addConstr(y[j] <= z[j] + model2_M * (1 - u[j][1]));
		/*model.addConstr(p[j] <= z[j] + max_c * (1 - u[j][0]));
		model.addConstr(y[j] <= z[j] + max_c * (1 - u[j][1]));*/
		model.addConstr(u[j][0] + u[j][1] >= 1);

		model.addConstr(y[j] >= c[j] - model2_jobs[j - 1].due_date);
		model.addConstr(0 >= y[j] - model2_M * (1 - v[j][0]));
		model.addConstr(c[j] - model2_jobs[j - 1].due_date >= y[j] - model2_M * (1 - v[j][1]));
	...
	try
	{
		...
		model2_result = model.get(GRB_DoubleAttr_ObjVal);
	}
	catch (GRBException err)
	{
		cout << "-------------" << endl;
...
int model2_solve()
{
	...
	memcpy(model2_jobs, job_set, sizeof(job_set));
	...
	model2_running_time = 1.0 * (t2.QuadPart - t1.QuadPart) / cpu_freq.QuadPart;
